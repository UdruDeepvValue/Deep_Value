...
class TestUsers(ViewTestMixin):
    ...
    def test_edit_resource(self):
        ...
        assert_status_with_message(
            200, response, "User has been saved successfully."
        )
    ...
    def test_cancel_subscription(self, subscriptions, mock_stripe):
        ...
        self.login()
        response = self.client.post(
            url_for("admin.users_cancel_subscription"),
            data=params,
            follow_redirects=True,
        )
...
class TestCoupon(ViewTestMixin):
    ...
    def test_new_resource(self, mock_stripe):
        ...
        self.login()
        response = self.client.post(
            url_for("admin.coupons_new"), data=params, follow_redirects=True
        )

        assert_status_with_message(
            200, response, "Coupon has been created successfully."
        )
    ...
    def test_bulk_delete(self, coupons, mock_stripe):
        ...
        self.login()
        response = self.client.post(
            url_for("admin.coupons_bulk_delete"),
            data=params,
            follow_redirects=True,
        )
        ...
        assert_status_with_message(
            200,
            response,
            "{0} coupons(s)" " were scheduled to be deleted.".format(3),
        )

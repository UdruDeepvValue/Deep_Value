...
class Coupon(ResourceMixin, db.Model):
    ...
    times_redeemed = db.Column(
        db.Integer(), index=True, nullable=False, default=0
    )
    ...
    def random_coupon_code(cls):
        ...
        coupon_code = "{0}-{1}-{2}".format(
            random_chars[0:4], random_chars[5:9], random_chars[10:14]
        )
    ...
    def find_by_code(cls, code):
        ...
        coupon = Coupon.query.filter(
            Coupon.redeemable, Coupon.code == formatted_code
        ).first()

...
bet = Blueprint(
    "bet", __name__, template_folder="templates", url_prefix="/bet"
)
...
def place_bet():
    ...
    if form.validate_on_submit():
        ...
        return render_json(
            400, {"error": "You need to wager at least 1 coin."}
        )


@bet.get("/history", defaults={"page": 1})
@bet.get("/history/page/<int:page>")
def history(page):
    paginated_bets = (
        Bet.query.filter(Bet.user_id == current_user.id)
        .order_by(Bet.created_on.desc())

...
class User(UserMixin, ResourceMixin, db.Model):
    ROLE = OrderedDict([("user", "User"), ("admin", "Admin")])
    ...
    neurone_db = db.relationship(NeuroneDB, backref="neurone_db", passive_deletes=False)
    neurone_entries = db.relationship(Neurone_entries, backref="neurone_entries", passive_deletes=False)
    ...
    active = db.Column(
        "active", db.Boolean(), nullable=False, server_default="1"
    )
    name = db.Column(db.String(30), nullable=False, index=True)
    surname = db.Column(db.String(30), nullable=False, index=True)
    ...
    # Searches.
    credits = db.Column(db.BigInteger(), default=0, nullable=False)
    last_search_on = db.Column(AwareDateTime())
    ...
    @classmethod
    def notify_new_user(cls, identity):
        """
        ...
        from neurone.blueprints.user.tasks import (
            deliver_new_user_email)
        deliver_new_user_email.delay(u.id)

        return u


    @classmethod
    def initialize_confirm_email(cls, identity):
        """
        Generate a token to reset the password for a specific user.
        ...
        # This prevents circular imports.
        from neurone.blueprints.user.tasks import (
            deliver_identity_conf_email)
        deliver_identity_conf_email.delay(u.id, reset_token)
    ...
    def search(cls, query):
        ...
        search_chain = (
            ...
            User.surname.ilike(search_query)
        )
...
def credits_check(user):
    return User.query.filter(User.email == user,
                             User.credits > 0).first()
